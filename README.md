<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1E1B4B,50:4C1D95,100:0F0C29&height=200&section=header&text=Sudipto%20Kumar&fontSize=46&fontColor=E9D5FF&animation=fadeIn&fontAlignY=35&desc=Building%20useful%20products%20with%20code%2C%20automation%2C%20and%20AI&descAlignY=58&descSize=16&descColor=C4B5FD" width="100%" alt="Sudipto Kumar"/>

<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&weight=600&size=21&duration=3000&pause=1000&color=A78BFA&center=true&vCenter=true&width=620&lines=BBA+Student+%E2%80%94+Finance+%26+Banking;Full-Stack+%26+Automation+Developer;AI+%2F+Prompt+Engineering+Enthusiast;Building+Systems%2C+Not+Just+Scripts" alt="Typing SVG"/>

<br/>

<img src="https://img.shields.io/badge/BBA%20Student-Finance%20%26%20Banking-1E1B4B?style=for-the-badge&logoColor=C4B5FD" alt="Academic badge"/>

<br/><br/>

<a href="https://linkedin.com/in/sudipto-kumar"><img src="https://img.shields.io/badge/LinkedIn-1E1B4B?style=for-the-badge&logo=linkedin&logoColor=C4B5FD" alt="LinkedIn"/></a>
<a href="mailto:sudipto.karn@gmail.com"><img src="https://img.shields.io/badge/Email-1E1B4B?style=for-the-badge&logo=gmail&logoColor=C4B5FD" alt="Email"/></a>
<a href="https://github.com/SudiptoKumar"><img src="https://img.shields.io/badge/GitHub-1E1B4B?style=for-the-badge&logo=github&logoColor=C4B5FD" alt="GitHub"/></a>
<a href="https://t.me/NewsroomHQ"><img src="https://img.shields.io/badge/Telegram-1E1B4B?style=for-the-badge&logo=telegram&logoColor=C4B5FD" alt="Telegram"/></a>

<br/><br/>

<a href="https://github.com/SudiptoKumar"><img src="https://img.shields.io/github/followers/SudiptoKumar?label=Followers&style=flat-square&color=1E1B4B&labelColor=0F0C29" alt="GitHub followers"/></a>
<a href="https://github.com/SudiptoKumar"><img src="https://komarev.com/ghpvc/?username=sudiptokumar&label=Profile%20Views&style=flat-square&color=1E1B4B" alt="Profile views"/></a>
<a href="https://github.com/SudiptoKumar/SudiptoKumar"><img src="https://img.shields.io/github/last-commit/SudiptoKumar/SudiptoKumar?label=README%20updated&style=flat-square&color=1E1B4B" alt="Last updated"/></a>

<br/><br/>

<details>
<summary><b>📖 Table of Contents</b></summary>
<br/>

- [About](#-about)
- [AI & ML Focus](#-ai--ml-focus)
- [Tech Stack](#️-tech-stack)
- [Featured Projects](#-featured-projects)
- [Experience](#-experience)
- [Achievements](#-achievements)
- [Certifications](#-certifications)
- [Coding Profiles](#-coding-profiles)
- [GitHub Analytics](#-github-analytics)
- [Current Focus](#-current-focus)
- [Connect](#-connect)

</details>

</div>

---

## 🧭 About

I build practical digital products that connect **software, automation, AI, and information**.

My current work focuses on **Telegram automation, news aggregation, bot systems, web development, and AI-assisted workflows**. I enjoy taking a repetitive process, designing the system behind it, and turning it into something that runs reliably with minimal manual work.

Alongside development, I study **Finance and Banking** as a BBA student, which shapes how I think about products, data, business problems, and decision-making — I bring a product and business lens to engineering, not just a technical one.

> **Build useful things. Automate the repetitive. Keep learning.**

**Open to:** Internships · Freelance projects · Open-source collaboration · Interesting problems to automate

---

## 🧠 AI & ML Focus

<div align="center">

| Domain | Focus Level | Details |
|---|---|---|
| Prompt Engineering | Practicing | Designing prompts and workflows for content filtering, summarization, and ranking |
| Workflow Automation | Practicing | Building pipelines that combine APIs, scheduling, and state management |
| Applied AI Integration | Learning | Wiring LLM/AI APIs into real products rather than one-off scripts |
| Data-Informed Decisions | Practicing | Finance background shapes how I evaluate product and system trade-offs |

</div>

---

## 🛠️ Tech Stack

**Languages**

<img src="https://skillicons.dev/icons?i=python,ts,js,kotlin&theme=dark" alt="Languages"/>

**Frontend**

<img src="https://skillicons.dev/icons?i=react,html,css&theme=dark" alt="Frontend"/>

**Backend, Data & Cloud**

<img src="https://skillicons.dev/icons?i=nodejs,dotnet,postgres,mysql,mongodb,firebase&theme=dark" alt="Backend and databases"/>

**Tooling & DevOps**

<img src="https://skillicons.dev/icons?i=linux,bash,git,postman,jest,grafana&theme=dark" alt="Tooling"/>

---

## 🚀 Featured Projects

<details>
<summary><b>📰 Newsroom Network</b></summary>
<br/>

A connected Telegram publishing ecosystem for focused news feeds — covering discovery, filtering, summarization, automation, and publishing.

| | |
|---|---|
| **Stack** | Python · Telegram Bot API · Automation pipelines |
| **Focus** | News discovery · filtering · summarization · publishing |
| **Repository** | [@NewsroomHQ](https://t.me/NewsroomHQ) |

</details>

<details>
<summary><b>🤖 Telegram Automation</b></summary>
<br/>

Bots and automation pipelines for collecting, processing, ranking, and forwarding content across Telegram.

| | |
|---|---|
| **Stack** | Python · APIs · State management · Scheduled workflows |
| **Focus** | Reliability and low-touch operation |
| **Repository** | *Private / in progress* |

</details>

<details>
<summary><b>🎮 Gaming Newsroom Bot</b></summary>
<br/>

An automated gaming-news pipeline built around source discovery, relevance filtering, and structured Telegram publishing.

| | |
|---|---|
| **Stack** | Python · Telegram Bot API |
| **Focus** | Source discovery · relevance filtering · structured publishing |
| **Repository** | [SudiptoKumar/GamingNewsroomBot](https://github.com/SudiptoKumar/GamingNewsroomBot) |

</details>

<details>
<summary><b>💼 Business News Bot</b></summary>
<br/>

A business-news aggregation project focused on discovering relevant stories and delivering them through an automated workflow.

| | |
|---|---|
| **Stack** | Python · News APIs · Automation |
| **Focus** | Story discovery · aggregation · delivery |
| **Repository** | [SudiptoKumar/BusinessNewsBot](https://github.com/SudiptoKumar/BusinessNewsBot) |

</details>

---

## 💼 Experience

<!-- Add your real roles below, one block per position. Example structure: -->

<details>
<summary><b>Your Job Title — Company Name</b></summary>
<br/>

*Month Year – Present*

Short description of the role.

**Scope of work**
- What you owned or built
- Key responsibility
- Notable outcome

`Skill` `Skill` `Skill`

</details>

*✏️ Replace the block above with your actual work experience — remove this section entirely if you don't have any yet.*

---

## 🏆 Achievements

<div align="center">

| Recognition | Details |
|---|---|
| *Add your achievement* | *Add a short description* |

</div>

*✏️ Replace with real recognitions — hackathon placements, published work, competition results, etc.*

---

## 📜 Certifications

**AWS** · **Oracle** · **NPTEL** · **Cisco**

*✏️ Add your certification badges here, grouped by issuing provider. Only include certifications you've actually earned — most providers give you an official badge/verification link you can embed directly.*

---

## 👨‍💻 Coding Profiles

<a href="#"><img src="https://img.shields.io/badge/LeetCode-1E1B4B?style=for-the-badge&logo=leetcode&logoColor=C4B5FD" alt="LeetCode"/></a>
<a href="#"><img src="https://img.shields.io/badge/GeeksforGeeks-1E1B4B?style=for-the-badge&logo=geeksforgeeks&logoColor=C4B5FD" alt="GeeksforGeeks"/></a>
<a href="#"><img src="https://img.shields.io/badge/HackerRank-1E1B4B?style=for-the-badge&logo=hackerrank&logoColor=C4B5FD" alt="HackerRank"/></a>
<a href="#"><img src="https://img.shields.io/badge/CodeChef-1E1B4B?style=for-the-badge&logo=codechef&logoColor=C4B5FD" alt="CodeChef"/></a>

*✏️ Swap the `#` links for your real profile URLs. Once you share your usernames, I can wire up live stats cards for these too.*

---

## 📊 GitHub Analytics

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=sudiptokumar&show_icons=true&hide_border=true&count_private=true&rank_icon=github&theme=nightowl&bg_color=00000000" alt="GitHub stats"/>
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=sudiptokumar&layout=compact&hide_border=true&langs_count=8&theme=nightowl&bg_color=00000000" alt="Top languages"/>

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=sudiptokumar&hide_border=true&theme=nightowl&background=00000000" alt="GitHub streak"/>

<br/>

<img src="https://github-profile-trophy.vercel.app/?username=sudiptokumar&theme=radical&no-frame=true&row=1&margin-w=8&column=7" alt="GitHub trophies"/>

<br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=sudiptokumar&bg_color=00000000&color=A78BFA&line=8B5CF6&point=E9D5FF&area=true&hide_border=true" alt="Contribution activity graph" width="95%"/>

<br/><br/>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/SudiptoKumar/SudiptoKumar/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/SudiptoKumar/SudiptoKumar/output/github-contribution-grid-snake.svg" />
  <img alt="A snake eating through the contribution graph" src="https://raw.githubusercontent.com/SudiptoKumar/SudiptoKumar/output/github-contribution-grid-snake.svg" width="100%"/>
</picture>

</div>

---

## 🎯 Current Focus

```yaml
current_focus:
  learning:
    - AI / LLM-assisted workflows
    - System design fundamentals
  building:
    - Telegram automation pipelines
    - News aggregation systems
  exploring:
    - Prompt engineering techniques
    - Cloud deployment practices
  open_to:
    - Internships
    - Freelance collaborations
    - Open-source contributions
```

---

## 📫 Connect

<div align="center">

<a href="https://linkedin.com/in/sudipto-kumar">
  <img src="https://img.shields.io/badge/LinkedIn-1E1B4B?style=for-the-badge&logo=linkedin&logoColor=C4B5FD" alt="LinkedIn"/>
</a>
<a href="https://twitter.com/sudiptokarn">
  <img src="https://img.shields.io/badge/X%20%2F%20Twitter-1E1B4B?style=for-the-badge&logo=x&logoColor=C4B5FD" alt="X / Twitter"/>
</a>
<a href="https://instagram.com/real.sudipto">
  <img src="https://img.shields.io/badge/Instagram-1E1B4B?style=for-the-badge&logo=instagram&logoColor=C4B5FD" alt="Instagram"/>
</a>
<a href="https://t.me/NewsroomHQ">
  <img src="https://img.shields.io/badge/Telegram-1E1B4B?style=for-the-badge&logo=telegram&logoColor=C4B5FD" alt="Telegram"/>
</a>
<a href="mailto:sudipto.karn@gmail.com">
  <img src="https://img.shields.io/badge/Email-1E1B4B?style=for-the-badge&logo=gmail&logoColor=C4B5FD" alt="Email"/>
</a>

</div>

<p align="right"><a href="#top">⬆ Back to top</a></p>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F0C29,50:4C1D95,100:1E1B4B&height=130&section=footer&reversal=true" width="100%"/>

### Building systems, not just scripts.

</div>
